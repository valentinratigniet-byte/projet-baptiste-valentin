...
| 8 | `erp-legacy/migrate_erp_to_target.py` | `choices_inv = {v: k for ...}` écrase silencieusement les collisions de noms normalisés identiques (dernier client_id gagne) — commentaire "collisions improbables" non vérifié formellement | Contribue au plafond de précision de la dette #7 ; masque la vraie cause si jamais elle change | Documenté et quantifié dans `docs/REFONTE-ERP.md` — corrigé par la dette #7 (SIRET), pas par ce script | Sprint 4 (MDM), avec #7 |
